...
export async function getAllFormTemplates() {
  try {
    ...
    const [forms, surveys] = await Promise.all([
      FormTemplate.find({
        userId: ownerId,
        $or: [{ type: "form" }, { type: { $exists: false } }, { type: null }],
      }).lean<LeanForm[]>(),
      FormTemplate.find({ type: "survey", userId: ownerId }).lean<LeanForm[]>(),
    ]);

    const data = [...forms, ...surveys].map((f) => ({
      id: f._id?.toString() ?? "",
      title: f.title,
      type: (typeof f.type === "string" ? f.type : "form") as "form" | "survey",
...
export async function getFormTemplateById(formId: string) {
  try {
    ...
    const form = await FormTemplate.findOne({ _id: objectId, userId: ownerId }).lean<LeanForm>();
    if (!form) {
...
export async function updateFormTemplateType(id: string, type: "form" | "survey") {
  try {
    await connectDB();
    const user = await getCurrentUser();
    if (!user?._id) throw new Error("User not authenticated");
    ...
    const res = await FormTemplate.updateOne(
      { _id: objectId, userId: ownerId },
      { $set: { type } }
    );

    if (res.matchedCount === 0) {
      return { success: false, error: "Form not found or not owned by user" };
    }
    ...
    return { success: false, error: "Failed to update form type" };
  }
}
