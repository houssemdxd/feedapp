...
export default function FormsDashboardPage() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    setError(null);
    try {
      // Utiliser la nouvelle API forms/analytics
      const response = await fetch("/api/forms/analytics");
  ...
  const handleDeleteAsset = useCallback(
    async (asset: EngagementAsset) => {
      const confirmed = window.confirm(
        `Delete ${asset.type.toLowerCase()} "${asset.title}" and all of its responses? This cannot be undone.`
      );
      if (!confirmed) {
        return;
      }
      ...
      try {
        ...
        const res = await fetch(`/api/forms/${asset.id}`, { method: "DELETE" });
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.error || `Unable to delete this ${asset.type.toLowerCase()}.`);
        }
        ...
        setDeleteError(
          deleteErr instanceof Error
            ? deleteErr.message
            : `Something went wrong while deleting this ${asset.type.toLowerCase()}.`
        );
      } finally {
        setDeletingId(null);
      }
                  ...
                  <TableRow key={asset.id}>
                    ...
                    <TableCell className="py-4">
                      <button
                        ...
                        disabled={deletingId === asset.id || loading}
