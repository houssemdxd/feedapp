...
export default function FormFill({ formId, allowUpdate = false }: { formId: string; allowUpdate?: boolean }) {
      ...
      try {
        ...
        try {
          ...
          if (meRes.ok) {
            ...
            if (me?.submitted) {
              // For surveys: allow update, for forms: block
              const isSurvey = json.form.type === "survey";
              if (isSurvey || allowUpdate) {
                setIsUpdate(true);
              } else {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-6 text-gray-900 dark:text-gray-100">
      ...
      {alreadySubmitted && data && data.form.type !== "survey" && (
        <div className="p-4 border rounded bg-yellow-50 text-yellow-800">
          Vous avez déjà répondu à ce formulaire. Vous ne pouvez pas le soumettre à nouveau.
        </div>
      )}

      {(isUpdate || (data && data.form.type === "survey" && alreadySubmitted)) && !submitted && (
        <div className="p-4 border rounded bg-blue-50 text-blue-800 dark:bg-blue-500/10 dark:text-blue-300">
          Vous avez déjà répondu à ce survey. Vous pouvez modifier vos réponses ci-dessous.
        </div>
      )}
      ...
      {submitted && (
        <div className="text-green-600">
          {isUpdate ? "Réponses mises à jour avec succès, merci." : "Réponses envoyées, merci."}
        </div>
      )}
      ...
      <div className="flex gap-3">
        <button
          ...
          disabled={!canSubmit || submitting || (alreadySubmitted && data && data.form.type !== "survey" && !allowUpdate)}
          className="px-4 py-2 rounded bg-blue-600 text-white disabled:opacity-60"
        >
          {submitting 
            ? (isUpdate || (data && data.form.type === "survey") ? "Mise à jour…" : "Envoi…") 
            : (isUpdate || (data && data.form.type === "survey" && alreadySubmitted) ? "Mettre à jour" : "Soumettre")}
        </button>
