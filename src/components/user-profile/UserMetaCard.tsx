// src/components/profile/UserMetaCard.tsx
"use client";
...
function CircleSkeleton({ size = 80 }: { size?: number }) {
  return (
    <span
      aria-hidden
      className="overflow-hidden rounded-full border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-white/10 animate-pulse"
      style={{ width: size, height: size, display: "inline-block" }}
    />
  );
}
function LineSkeleton({ width = "18ch", className = "" }: { width?: string; className?: string }) {
  return (
    <span
      aria-hidden
      className={`inline-block h-4 rounded bg-gray-100 dark:bg-white/10 animate-pulse ${className}`}
      style={{ width }}
    />
  );
}
      ...
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            <div className="order-3 xl:order-2">
              <h4 className="mb-2 text-lg font-semibold text-center text-gray-800 dark:text-white/90 xl:text-left">
                {loading ? <LineSkeleton width="16ch" /> : display.name}
              </h4>
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              {/* Socials — static defaults; keep/edit as needed */}
              <a
                target="_blank"
                rel="noreferrer"
          ...
          <button
            ...
            disabled={loading}
          >
            <svg className="fill-current" width="18" height="18" viewBox="0 0 18 18" aria-hidden>
              <path
                fillRule="evenodd"
                clipRule="evenodd"
